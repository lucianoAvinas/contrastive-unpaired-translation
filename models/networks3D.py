...
class PatchSampleF(nn.Module):
    ...
    def forward(self, feats, num_patches=64, patch_ids=None):
        ...
        for feat_id, feat in enumerate(feats):
            B, Z, H, W = feat.shape[0], feat.shape[2], feat.shape[3], feat.shape[4]
            ...
            if num_patches == 0:
                x_sample = x_sample.permute(0, 2, 1).reshape([B, x_sample.shape[-1], Z, H, W])
...
class StyleEncoder(nn.Module):
    def __init__(self, n_downsample, input_dim, dim, style_dim, norm, activ, vae=False):
        ...
        for i in range(2):
            self.model += [Conv3dBlock(dim, 2 * dim, 4, 2, 1, norm=norm, activation=activ, pad_type='replicate')]
            dim *= 2
        for i in range(n_downsample - 2):
            self.model += [Conv3dBlock(dim, dim, 4, 2, 1, norm=norm, activation=activ, pad_type='replicate')]
...
class Decoder_all(nn.Module):
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0):
        ...
        for i in range(n_upsample):
            block = [Upsample2(scale_factor=2), Conv3dBlock(dim + nz, dim // 2, 5, 1, 2, norm='ln', activation=activ, pad_type='replicate')]
            setattr(self, 'block_{:d}'.format(self.n_blocks), nn.Sequential(*block))
            self.n_blocks += 1
            dim //= 2
        # use reflection padding in the last conv layer
        setattr(self, 'block_{:d}'.format(self.n_blocks), Conv3dBlock(dim + nz, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type='replicate'))
        self.n_blocks += 1
    ...
    def __init__(self, n_upsample, n_res, dim, output_dim, norm='batch', activ='relu', pad_type='zero', nz=0):
        ...
        for i in range(n_upsample):
            if i == 0:
                ...
                input_dim = dim
            self.model += [Upsample2(scale_factor=2), Conv3dBlock(input_dim, dim // 2, 5, 1, 2, norm='ln', activation=activ, pad_type='replicate')]
            dim //= 2
        # use reflection padding in the last conv layer
        self.model += [Conv3dBlock(dim, output_dim, 7, 1, 3, norm='none', activation='tanh', pad_type='replicate')]
        self.model = nn.Sequential(*self.model)
...
class Conv3dBlock(nn.Module):
    def __init__(self, input_dim, output_dim, kernel_size, stride,
                 padding=0, norm='none', activation='relu', pad_type='replicate'):
...
class ResnetGenerator(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate', no_antialias=False, no_antialias_up=False, opt=None):
        ...
        model = [nn.Conv3d(input_nc, ngf, kernel_size=7, padding=3, bias=use_bias, padding_mode='replicate'),
                 norm_layer(ngf),
                 nn.ReLU(True)]
        ...
        for i in range(n_downsampling):  # add upsampling layers
            ...
            if no_antialias_up:
                ...
                model += [Upsample(ngf * mult),
                          ...
                          norm_layer(int(ngf * mult / 2)),
                          nn.ReLU(True)]
        model += [nn.Conv3d(ngf, output_nc, kernel_size=7, padding=3, padding_mode='replicate')]
        model += [nn.Tanh()]
...
class ResnetDecoder(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm3d, use_dropout=False, n_blocks=6, padding_type='replicate', no_antialias=False):
...
class ResnetBlock(nn.Module):
    ...
    def build_conv_block(self, dim, padding_type, norm_layer, use_dropout, use_bias):
        ...
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=use_bias, padding_mode=padding_type), norm_layer(dim), nn.ReLU(True)]
        if use_dropout:
        ...
        if padding_type == 'reflect':
            ...
            raise NotImplementedError('padding [%s] is not implemented' % padding_type)
        """
        conv_block += [nn.Conv3d(dim, dim, kernel_size=3, padding=1, bias=use_bias, padding_mode=padding_type), norm_layer(dim)]
...
class PatchDiscriminator(NLayerDiscriminator):
    ...
    def forward(self, input):
        B, C, Z, H, W = input.size(0), input.size(1), input.size(2), input.size(3), input.size(4)
        size = 16
        V = Z // size
        Y = H // size
        X = W // size
        input = input.view(B, C, V, size, Y, size, X, size)
        input = input.permute(0, 2, 4, 6, 1, 3, 5, 7).contiguous().view(B * V * Y * X, C, size, size, size)

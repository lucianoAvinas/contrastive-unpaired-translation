...
def upfirdn3d_native(
    ...
    _, minor, in_z, in_h, in_w = input.shape
    kernel_z, kernel_h, kernel_w = kernel.shape

    out = input.view(-1, minor, in_z, 1, in_h, 1, in_w, 1)
    out = F.pad(out, [0, up_z - 1, 0, 0, 0, up_x - 1, 0, 0, 0, up_y - 1, 0, 0])
    out = out.view(-1, minor, in_z * up_z, in_h * up_y, in_w * up_x)

    out = F.pad(
        out, [max(pad_x0, 0), max(pad_x1, 0), max(pad_y0, 0), max(pad_y1, 0), max(pad_z0, 0), max(pad_z1, 0)]
    )
    out = out[
        :,
        :,
        max(-pad_z0, 0): out.shape[2] - max(-pad_z1, 0),
        max(-pad_y0, 0): out.shape[3] - max(-pad_y1, 0),
        max(-pad_x0, 0): out.shape[4] - max(-pad_x1, 0),
    ]

    # out = out.permute(0, 3, 1, 2)
    out = out.reshape(
        [-1, 1, in_z * up_z + pad_z0 + pad_z1, in_h * up_y + pad_y0 + pad_y1, in_w * up_x + pad_x0 + pad_x1]
    )
    w = torch.flip(kernel, [0, 1]).view(1, 1, kernel_z, kernel_h, kernel_w)
    out = F.conv2d(out, w)
    out = out.reshape(
        -1,
        minor,
        in_z * up_z + pad_z0 + pad_z1 - kernel_z + 1,
...
def upfirdn3d(input, kernel, up=1, down=1, pad=(0, 0)):
    return upfirdn3d_native(input, kernel, up, up, up, down, down, down, pad[0], pad[1], pad[0], pad[1], pad[0], pad[1])
...
class ModulatedConv3d(nn.Module):
    ...
    def forward(self, input, style):
        batch, in_channel, z, height, width = input.shape

        if style is not None:
            style = self.modulation(style).view(batch, 1, in_channel, 1, 1, 1)
        else:
            style = torch.ones(batch, 1, in_channel, 1, 1, 1).cuda()
        ...
        if self.upsample:
            input = input.view(1, batch * in_channel, z, height, width)
            weight = weight.view(
                batch, self.out_channel, in_channel, self.kernel_size, self.kernel_size, self.kernel_size
            )
            ...
            out = F.conv_transpose3d(input, weight, padding=0, stride=2, groups=batch)
            _, _, z, height, width = out.shape
            out = out.view(batch, self.out_channel, z, height, width)
            out = self.blur(out)

        elif self.downsample:
            input = self.blur(input)
            _, _, z, height, width = input.shape
            input = input.view(1, batch * in_channel, z, height, width)
...
class NoiseInjection(nn.Module):
    ...
    def forward(self, image, noise=None):
        if noise is None:
            batch, _, z, height, width = image.shape
            noise = image.new_empty(batch, 1, z, height, width).normal_()
...
class StyleGAN2Discriminator(nn.Module):
    ...
    def forward(self, input, get_minibatch_features=False):
        if "patch" in self.opt.netD and self.opt.D_patch_size is not None:
            z, h, w = input.size(2), input.size(3), input.size(4)
            v = torch.randint(z - self.opt.D_patch_size, ())
            y = torch.randint(h - self.opt.D_patch_size, ())
            x = torch.randint(w - self.opt.D_patch_size, ())
            input = input[:, :, v:v + self.opt.D_patch_size, y:y + self.opt.D_patch_size, x:x + self.opt.D_patch_size]
        out = input
        ...
        batch, channel, z, height, width = out.shape

        if False and "tile" in self.opt.netD:
            group = min(batch, self.stddev_group)
            stddev = out.view(
                group, -1, 1, channel // 1, z, height, width
            )
            stddev = torch.sqrt(stddev.var(0, unbiased=False) + 1e-8)
            stddev = stddev.mean([2, 3, 4], keepdim=True).squeeze(2)
            stddev = stddev.repeat(group, 1, z, height, width)
...
class TileStyleGAN2Discriminator(StyleGAN2Discriminator):
    def forward(self, input):
        B, C, Z, H, W = input.size(0), input.size(1), input.size(2), input.size(3)
